...
class RVTAgent:
    ...
    def update(
        ...
        assert list(replay_sample["rot_grip_action_indicies"].shape[1:]) == [4]
        assert list(replay_sample["ignore_collisions"].shape[1:]) == [1]
        assert list(replay_sample["gripper_pose"].shape[1:]) == [7]
        assert list(replay_sample["lang_goal_embs"].shape[1:]) == [77, 512]
        assert list(replay_sample["low_dim_state"].shape[1:]) == [self._net_mod.proprio_dim]

        # sample
        action_rot_grip = replay_sample["rot_grip_action_indicies"].int()  # (b, 4) of int
        action_ignore_collisions = replay_sample["ignore_collisions"].int()  # (b, 1) of int
        ...
        lang_goal_embs = replay_sample["lang_goal_embs"].float() # (b, 77, 512)
        # tasks = replay_sample["tasks"]
        tasks = [s.decode() for s in replay_sample["tasks"]]

        proprio = arm_utils.stack_on_channel2(replay_sample["low_dim_state"])  # (b, 4)
        return_out = {}

        obs, pcd = peract_utils._preprocess_inputs2(replay_sample, self.cameras)
    ...
    @torch.no_grad()
    def act(self, step: int, observation: dict, deterministic=True) -> ActResult:
        if self.add_lang:
    ...
    def get_action_trans(self, wpt_local, pts, out, dyn_cam_info, dims):
        bs, nc, h, w = dims
        wpt_img = self._net_mod.get_pt_loc_on_img(wpt_local.unsqueeze(1), dyn_cam_info=dyn_cam_info, out=None)
